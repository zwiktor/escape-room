...
async def check_status(db: AsyncSession, user: User, story_id: int):
    data = {'status': 'new',
            'story_access': {'purchase_date': None,
                             'current_attempt': None}
            }
    attempt = None
    story_access = await get_instance(session=db, model=StoryAccess, story_id=story_id,
                                      user_id=user.id)
    if story_access:
        attempt = await get_last_instance(session=db, model=Attempt, order_by=Attempt.start_date,
